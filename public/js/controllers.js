                ...
                .success(function(data) {
                    ...
                    if(array.length>0){
                        for(var i=0;i<array.length;i++){
                            ...
                            var string = "<div class='thinD' style='border-top: none'> <label class='rightB' style='font-size:8px;width: 8%;float: left;height: 15px'>"+d.ShipperNo+"</label> <label class='rightB' style='font-size:8px;width: 20%;float: left;height: 15px'>"+d.DateTime+"</label> <label class='rightB' style='font-size:8px;width: 10%;float: left;height: 15px'>"+d.ShiftTime+"</label> <label class='rightB' style='font-size:8px;width: 30%;float: left;height: 15px'>"+d.User+"</label> <label style='font-size:8px;width: 15%;float: left;padding: 3px;;height: 15px'>"+d.Weight+" (Kg.)</label> </div>";
                            stringMaster += string;
                    ...
                    if(batch.Print == 0)
                    {
                        htmltext = htmltext.replace('{{batch.MfgNo}}','');
                        htmltext = htmltext.replace('{{batch.ProductName}}','');
                        htmltext = htmltext.replace('{{batch.MfgDate}}','');
                        htmltext = htmltext.replace('{{batch.ExpDate}}','');
                        htmltext = htmltext.replace('{{batch.MinShipperWt}}','');
                        htmltext = htmltext.replace('{{batch.MaxShipperWt}}','');
                        htmltext = htmltext.replace('{{batch.MinShipperWt}}','');
                        htmltext = htmltext.replace('{{batch.MaxShipperWt}}','');
                        htmltext = htmltext.replace('{{loop}}',stringMaster);
                    }
